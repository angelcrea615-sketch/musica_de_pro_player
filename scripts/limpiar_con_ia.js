...
async function obtenerModeloValido(apiKey) {
    try {
        ...
        if (data && data.models) {
            // Buscamos el primer modelo que soporte generación de contenido
            ...
            if (modelo) {
                ...
                return modelo.name; // Ej: "models/gemini-1.5-flash" u otro disponible
            }
        }
    } catch (e) {
        console.warn("⚠️ No se pudo listar automáticamente, usando respaldo.");
    }
...
async function corregirConIA(cancion, apiKey, nombreModelo) {
    ...
    while (intentos < 3) {
        try {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1/${nombreModelo}:generateContent?key=${apiKey}`, {
                method: 'POST',
